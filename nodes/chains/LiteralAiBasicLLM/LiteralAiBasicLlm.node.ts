...
export class LiteralAiBasicLlm implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Literal AI Basic LLM',
		name: 'literalAiBasicLlm',
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				const variablesParameter = this.getNodeParameter('variables', i, '{}') as string;

				const result = await client
					.thread({ name: this.getWorkflow().name })
					.wrap(async () => {
						...
						const variables = { 
							...JSON.parse(variablesParameter),
						};

						if (!prompt.settings.model) {
							throw new NodeOperationError(this.getNode(), 'Model is not specified');
						}
